...
Chart Settings:
 ...
 - Namespaced _Role_ and _RoleBinding_ RBAC resources are not yet suported by this chart, however the way Secrets-Manager works is to create the target Secret in the same namespace as the `SecretDefinition` so this should not be an issue.
